;
; RC5-72 Dual key core - MPC603e/MPC74x/MPC75x (MacOS ABI)
; TAB = 4
;
...
;
; This implementation is optimized for MPC603e and MPC740/MPC750 (G3). 
; The core checks 2 keys per loop using the integer units.
;
; Specifications :
;	MPC603e
;		- Fetch/dispatch/retire 2 instructions per clock cycle.
;		- 2 stages LSU, 1 IU.
;		- SRU can handle add* instructions
;		- Only one rotlw/rotlwi instruction can be dispatched
;		  per clock cycle.
;
;	MPC74x/75x (G3)
;		- Fetch 4 instructions per clock cycle.
;		- Dispatch/retire 2 instructions per clock cycle.
;		- 2 stages LSU, 2 IU.
;
;	Store and load instructions are likely to cause Completion
;	Queue stalls since the LSU has 2 pipeline stages.
;
; Dependencies : 
;
...
;		and lo values).
;
; $Id: r72-KKS2pipes.osx.s,v 1.1.2.1 2003/01/20 00:25:58 mfeiri Exp $
;
; $Log: r72-KKS2pipes.osx.s,v $
; Revision 1.1.2.1  2003/01/20 00:25:58  mfeiri
; New core by kakace for 603 and G3
;
; Revision 1.2  2003/01/12 20:53:47  kakace
; Fixed key.mid incrementation
;
; Revision 1.1  2003/01/12 19:44:20  kakace
...
;	Linkage area
	...
	linkageArea	=	24			; Size of the linkageArea.


; Local variables area
; Offsets (from the callee's SP) to the local variable space.

	saveR2	=	linkageArea		; Storage for r2 (defined as volatile under
								; MacOS X, but usually points to RTOC).
	Key		=	saveR2 + 4		; key.hi, key.mid, key.lo
...
;============================================================================
;
; s32 rc5_72_unit_func_Dual (RC5_72UnitWork *rc5_72unitwork (r3), 
...
inner_loop_lo:
		...
		lis		r13,hi16(P)			; S[0] = P
		ori		r13,r13,lo16(P)
		add		r9,r13,r10			; S[1] = P + Q

		lwz		r11,Key+8(r1)		; key.lo
		rotlwi	r13,r13,3			; a = s[0] = ROTL3(...)

		add		r11,r11,r13			; key.lo += a
		rotlw	r11,r11,r13			; b = L[0] = ROTL(...)

		add		r24,r13,r11			; a + b
		add		r14,r9,r24			; s[1] = S[1] + (a + b)
		rotlwi	r14,r14,3			; a = s[1] = ROTL3(...)
		
		add		r9,r9,r10			; S[2] = S[1] + Q
...
inner_loop_mid:
		...
		add		r24,r12,r14			; a + b
		add		r15,r24,r9			; s[2] = S[2] + (a + b)
		rotlwi	r15,r15,3			; a = s[2] = ROTL3(...)

		add		r9,r9,r10			; S[3] = S[2] + Q
...
inner_loop_hi:
	...
	.macro	mix1					; l0[j], l1[j], S_kk
		add		$0,$0,r24			; l0[j] += t0
		add		$1,$1,r25			; l1[j] += t1
		rotlw	$0,$0,r24			; b0 = l0[j] = ROTL(...)
		stw		r22,$2(r1)			; save s0[k]
		rotlw	$1,$1,r25			; b1 = l1[j] = ROTL(...)
		stw		r23,$2+4(r1)		; save s1[k]
		
		add		r24,r22,$0			; t0 = a0 + b0
		add		r25,r23,$1			; t1 = a1 + b1
		add		r22,r9,r24			; s0[i] = S[i] + t0
		add		r23,r9,r25			; s1[i] = S[i] + t1
		rotlwi	r22,r22,3			; a0 = s0[i] = ROTL3(...)
		add		r9,r9,r10			; S[i+1] = S[i] + Q
	...
	.endmacro
		...
		mix1	r20,r21,S_05		; Step  1.6 : l[2] and s[6]
		mix1	r16,r17,S_06		; Step  1.7 : l[0] and s[7]
		mix1	r18,r19,S_07		; Step  1.8 : l[1] and s[8]
		mix1	r20,r21,S_08		; Step  1.9 : l[2] and s[9]
		mix1	r16,r17,S_09		; Step 1.10 : l[0] and s[10]
		mix1	r18,r19,S_10		; Step 1.11 : l[1] and s[11]
		mix1	r20,r21,S_11		; Step 1.12 : l[2] and s[12]
		mix1	r16,r17,S_12		; Step 1.13 : l[0] and s[13]
		mix1	r18,r19,S_13		; Step 1.14 : l[1] and s[14]
		mix1	r20,r21,S_14		; Step 1.15 : l[2] and s[15]
		mix1	r16,r17,S_15		; Step 1.16 : l[0] and s[16]
		mix1	r18,r19,S_16		; Step 1.17 : l[1] and s[17]
		mix1	r20,r21,S_17		; Step 1.18 : l[2] and s[18]
		mix1	r16,r17,S_18		; Step 1.19 : l[0] and s[19]
		mix1	r18,r19,S_19		; Step 1.20 : l[1] and s[20]
		mix1	r20,r21,S_20		; Step 1.21 : l[2] and s[21]
		mix1	r16,r17,S_21		; Step 1.22 : l[0] and s[22]
		mix1	r18,r19,S_22		; Step 1.23 : l[1] and s[23]
		mix1	r20,r21,S_23		; Step 1.24 : l[2] and s[24]
		mix1	r16,r17,S_24		; Step 1.25 : l[0] and s[25]
									; Still update r9 (compute S[26])
	...
	.macro	mix2					; l0[j], l1[j], S_kk[]
		add		$0,$0,r24			; l0[j] += t0
		lwz		r30,$2+8(r1)		; pre-load s0[i+1]
		add		$1,$1,r25			; l1[j] += t1
		lwz		r31,$2+12(r1)		; pre-load s1[i+1]
		rotlw	$0,$0,r24			; b0 = l0[j] = ROTL(...)
		stw		r22,$2(r1)			; save s0[i]
		rotlw	$1,$1,r25			; b1 = l1[j] = ROTL(...)
		stw		r23,$2+4(r1)		; save s1[i]
	...
	.macro	mix3	; l0[i-1], l1[i-1], l0[i], l1[i], l0[i+1], l1[i+1], S_xx
					;    $0       $1      $2     $3      $4       $5     $6
		...
		lwz		r30,$6(r1)			; pre-load s0[j+1]
		rotlwi	r23,r23,3			; a1 = s1[j] = ROTL3(...)
		lwz		r31,$6+4(r1)		; pre-load s1[j+1]
		...
		add		$2,$2,r24			; l0[i] += t0
		add		$3,$3,r25			; l1[i] += t1
		rotlw	$2,$2,r24			; b0 = l0[i] = ROTL(...)
		add		r26,r26,r22			; A0 += s0[j]
		rotlw	$3,$3,r25			; b1 = l1[i] = ROTL(...)
		add		r27,r27,r23			; A1 += s1[j]

		
		add		r24,r22,$2			; t0 = a0 + b0
		xor		r28,r28,r26			; B0 = B0 ^ A0
		add		r25,r23,$3			; t1 = a1 + b1
		...
		lwz		r30,$6+8(r1)		; pre-load s0[j+2]
		rotlwi	r23,r23,3			; a1 = s1[j+1] = ROTL3(...)
		lwz		r31,$6+12(r1)		; pre-load s1[j+2]
		rotlw	r29,r29,r27			; B1 = ROTL(B1, A1)
		
		add		r24,r22,$2			; t0 = a0 + b0
		add		r25,r23,$3			; t1 = a1 + b1
		add		$4,$4,r24			; l0[i+1] += t0
		add		$5,$5,r25			; l1[i+1] += t1
		rotlw	$4,$4,r24			; b0 = l0[i+1] = ROTL(...)
		add		r28,r28,r22			; B0 += s0[j+1]
		rotlw	$5,$5,r25			; b1 = l1[i+1] = ROTL(...)
