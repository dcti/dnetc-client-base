...
static bool init_rc5_72_ocl_4pipe(u32 device)
{
  ...
  if(ocl_context[device].coreID!=CORE_4PIPE)
  ...
  if(!BuildCLProgram(device, ocl_rc572_4pipe_src, "ocl_rc572_4pipe"))
	  return false;
  ...
  ocl_context[device].coreID=CORE_4PIPE;
  return true;
}
...
s32 rc5_72_unit_func_ocl_4pipe(RC5_72UnitWork *rc5_72unitwork, u32 *iterations, void *)
{
  ...
  if (ocl_context[deviceID].coreID!=CORE_4PIPE)
  {
    init_rc5_72_ocl_4pipe(deviceID);
    if(ocl_context[deviceID].coreID!=CORE_4PIPE) {
