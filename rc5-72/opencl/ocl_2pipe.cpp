...
static bool init_rc5_72_ocl_2pipe(ocl_context_t *cont)
{
  if(!cont->active)
  {
  ...
  if(!BuildCLProgram(cont, ocl_rc572_2pipe_src, "ocl_rc572_2pipe"))
	  return false;
...
static bool ClearOutBuffer(const cl_mem buffer, ocl_context_t *cont)
{
  ...
  if(ocl_diagnose(status, "unmapping output buffer (1)", cont->clientDeviceNo) != CL_SUCCESS)
	return false;
  return true;
}
...
static s32 ReadResults(const cl_mem buffer, u32 *CMC, u32 *iters_done, ocl_context_t *cont)
{
  ...
  *iters_done = 0xffffffff;
  ...
  if(found)
  {
    ...
    status = clEnqueueUnmapMemObject(cont->cmdQueue, buffer, outPtr, 0, NULL, NULL);
    if(ocl_diagnose(status, "unmapping output buffer (2)", cont->clientDeviceNo) != CL_SUCCESS)
	  return -1;
    if(fullmatch)
      return 1;
    return 0;
  ...
  status = clEnqueueUnmapMemObject(cont->cmdQueue, buffer, outPtr, 0, NULL, NULL);
  if(ocl_diagnose(status, "unmapping output buffer (3)", cont->clientDeviceNo) != CL_SUCCESS)
    return -1;
  return 0; 
}
...
//		Log("CMC at:%x:%x:%x, plain:%x:%x, cypher=%x:%x\n", hi,mid,lo,tmp_unit.plain.hi,tmp_unit.plain.lo,tmp_unit.cypher.hi,tmp_unit.cypher.lo);
  ...
  memmove(rc5_72unitwork, &tmp_unit, sizeof(RC5_72UnitWork));
  rc5_72unitwork->optimal_timeslice_increment = cont->runSizeMultiplier*2;
