...
**
** This core is based on the 2-pipes core for Sparc processors written by
** Didier Levet (kakace@wanadoo.fr). It is designed for R10000 and
** higher, but has maybe also advantages on older processors.
**
** R10000 architecture :
**	- 2 independant integer units. (result delay 1 cycle except
**	  multiply and divide)
**	- 2 fp units (not useful here)
**	- 1 load/store unit. (result delay 2 cycles)
**	- can dispatch 4 instructions per clock.
**	- has dynamic scheduling and out-of-order execution
**	- integer, fp and l/s queue have 16 entrys each, this is the
**	  range in which out-of-order execution can operate
**
** Bottlenecks :
**	- only Alu 1 can be used for shifts
**	- only Alu 2 can be used for multiply and divide
**	- 2 shifts cannot be dispatched in the same cycle,
**	  theoretically this should not matter because of
**	  out-of-order execution, but in reality it helps not
**	  to have 2 shifts following each other
**	- only 1 load/store can be dispatched in the same
**	  cycle, here applies the same as for shifts
*/
...
extern "C" s32 CDECL rc5_72_unit_func_MIPS_2 ( RC5_72UnitWork *, u32 *, void * );
  ...
  #define ASM_SUB(res, op1, op2) \
    __asm__ volatile ("subu %0, %1, %2" : "=r" (res) : "r" (op1), "r" (op2));
  ...
  #define ASM_SL(res, op, dist) \
    __asm__ volatile ("sllv %0, %1, %2" : "=r" (res) : "r" (op), "r" (dist));
  #define ASM_SR(res, op, dist) \
    __asm__ volatile ("srlv %0, %1, %2" : "=r" (res) : "r" (op), "r" (dist));
...
s32 CDECL rc5_72_unit_func_MIPS_2 (RC5_72UnitWork *rc5_72unitwork, u32 *iterations, void * /*memblk*/)
{
	u32 Sa[52], Sb[52];
	u32 kiter;
