...
[GLOBAL _rc5_72_unit_func_go_2d]
[GLOBAL rc5_72_unit_func_go_2d]
...
%define S1(N)	[work_S+((N)*4)]
%define S2(N)	[work_S+((26+N)*4)]
%define S3(N)	[work_S+((26*2+N)*4)]
%define S4(N)	[work_S+((26*3+N)*4)]
%define L3(N)	[work_L+((N)*4)]
%define L4(N)	[work_L+((N+3)*4)]
%define L3e(N)	[work_L+((N)*4)+r15]
%define S1e(N)	[rbp+((N)*4)]
%define S2e(N)	[rbp+((N+26)*4)]
%define S3e(N)	[rbp+((N)*4)]
...
align 16
startseg:
_rc5_72_unit_func_go_2d:
rc5_72_unit_func_go_2d:
...
key_setup_1_bigger_loop:
	...
	lea	rbp,[work_S+26*2*4]			;pointer to S3
	xor	r15,r15					;pipe 3 destination flag
				lea	B3_2,[B1_2+4]
...
key_setup_1_inner_loop:
	...
	lea	ecx,[A2+B2_2]		     		;S2[3]+L2[2]
				mov	B3_1,[work_pre3_r1]
	mov	S2(3),A2
	mov	B1_1,[work_pre3_r1]	     		;L1(1)
	mov	B2_1,[work_pre3_r1]			;L2(1)

;------------------Round 1(4)-------------------------
	lea	A1,[A1+B1_0+S_not(4)]			;S1[3]+L1[0]+S_not(4)
	...
	rol	A1,3			     		;S1[4]
				...
				add	B3_0,ecx
	lea	A2,[A2+B2_0+S_not(4)]			;S2[3]+L2[0]+S_not(4)
	...
	rol	A2,3			     		;S2[4]
				lea	A3,[A3+B3_0+S_not(4)]

	rol	B1_1,cl 		     		;L1[1]
	lea	ecx,[A2+B2_0]		 		;S2[4]+L2[0]
	mov	S2(4),A2
	;---
	lea	A1,[A1+B1_1+S_not(5)]			;S1[4]+L1[1]+S_not(5)
	...
	rol	A1,3			     		;S1[5]
				add	B3_1,ecx
	lea	A2,[A2+B2_1+S_not(5)]			;S2[3]+L2[0]+S_not(4)
	...
	add	B1_2,ecx
	rol	A2,3
				lea	A3,[A3+B3_1+S_not(5)]

	rol	B1_2,cl
	lea	ecx,[A2+B2_1]
	mov	S2(5),A2
;---------------------Round 1(6)-------------------
	lea	A1,[A1+B1_2+S_not(6)]
	add	B2_2,ecx
				rol	A3,3
	...
	add	B1_0,ecx
	rol	A2,3
				lea	A3,[A3+B3_2+S_not(6)]

	rol	B1_0,cl
	lea	ecx,[A2+B2_2]
	mov	S2(6),A2
	;---
	lea	A1,[A1+B1_0+S_not(7)]
	add	B2_0,ecx
				rol	A3,3
	...
	add	B1_1,ecx
	rol	A2,3
				lea	A3,[A3+B3_0+S_not(7)]

	rol	B1_1,cl
	lea	ecx,[A2+B2_0]
	mov	S2(7),A2
;---------------------Round 1(8)-------------------
	lea	A1,[A1+B1_1+S_not(8)]
	add	B2_1,ecx
				rol	A3,3
	...
	add	B1_2,ecx
	rol	A2,3
				lea	A3,[A3+B3_1+S_not(8)]

	rol	B1_2,cl	
	lea	ecx,[A2+B2_1]
	mov	S2(8),A2
	;--- 
	lea	A1,[A1+B1_2+S_not(9)]
	add	B2_2,ecx
				rol	A3,3
	...
	add	B1_0,ecx
	rol	A2,3
				lea	A3,[A3+B3_2+S_not(9)]

	rol	B1_0,cl
	lea	ecx,[A2+B2_2]
	mov	S2(9),A2
;---------------------Round 1(10)-------------------
	lea	A1,[A1+B1_0+S_not(10)]
	add	B2_0,ecx
				rol	A3,3
	...
	add	B1_2,ecx
	rol	A2,3
				lea	A3,[A3+B3_1+S_not(11)]

	rol	B1_2,cl
	lea	ecx,[A2+B2_1]
	mov	S2(11),A2
;---------------------Round 1(12)-------------------
	lea	A1,[A1+B1_2+S_not(12)]
	add	B2_2,ecx
				rol	A3,3
	...
	add	B1_0,ecx
	rol	A2,3
				lea	A3,[A3+B3_2+S_not(12)]

	rol	B1_0,cl
	lea	ecx,[A2+B2_2]
	mov	S2(12),A2
	;---
	lea	A1,[A1+B1_0+S_not(13)]
	add	B2_0,ecx
				rol	A3,3
	...
	add	B1_1,ecx
	rol	A2,3
				lea	A3,[A3+B3_0+S_not(13)]

	rol	B1_1,cl
	lea	ecx,[A2+B2_0]
	mov	S2(13),A2
;---------------------Round 1(14)-------------------
	lea	A1,[A1+B1_1+S_not(14)]
	add	B2_1,ecx
				rol	A3,3
	...
	add	B1_2,ecx
	rol	A2,3
				lea	A3,[A3+B3_1+S_not(14)]

	rol	B1_2,cl
	lea	ecx,[A2+B2_1]
	mov	S2(14),A2
	;---
	lea	A1,[A1+B1_2+S_not(15)]
	add	B2_2,ecx
				rol	A3,3
	...
	add	B1_0,ecx
	rol	A2,3
				lea	A3,[A3+B3_2+S_not(15)]

	rol	B1_0,cl
	lea	ecx,[A2+B2_2]
	mov	S2(15),A2
;---------------------Round 1(16)-------------------
	lea	A1,[A1+B1_0+S_not(16)]
	add	B2_0,ecx
				rol	A3,3
	...
	add	B1_2,ecx
	rol	A2,3
				lea	A3,[A3+B3_1+S_not(17)]

	rol	B1_2,cl
	lea	ecx,[A2+B2_1]
	mov	S2(17),A2
;---------------------Round 1(18)-------------------
	lea	A1,[A1+B1_2+S_not(18)]
	add	B2_2,ecx
				rol	A3,3
...
;---------------------Round 2-Last-------------
	lea	ecx,[A2+B2_2]
	mov	S2(25),A2
				mov	S3e(25),A3	;ssssssssssssssssssssssss
				mov	L3e(0),B3_0
				mov	L3e(1),B3_1
				mov	L3e(2),B3_2
...
_NextKey:
	...
	lea	rbp,[work_S+(3*26)*4]
	mov	r15,12
	lea	B3_2,[B1_2+3]
	jmp	key_setup_1_inner_loop
align 16
new_run:
	lea	rbp,[work_S+2*26*4]
	xor	r15,r15
	lea	B3_2,[B1_2+4]
	jmp	key_setup_1_inner_loop
