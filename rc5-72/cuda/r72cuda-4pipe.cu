 ...
 *
 * $Id: r72cuda-4pipe.cu,v 1.16 2009/04/29 09:30:21 andreasb Exp $
*/
...
static s32 CDECL rc5_72_run_cuda_4(RC5_72UnitWork *rc5_72unitwork, u32 *iterations, int device, u32 num_threads, int waitmode)
{
  ...
  if (cudaStreamCreate(&core) != cudaSuccess || core == -1) {
    failed = "cudaStreamCreate";
    goto error_exit;
  }

  if (cudaEventCreate(&start) != cudaSuccess || start == -1) {
    failed = "cudaEventCreate";
    goto error_exit;
  }

  if (cudaEventCreate(&stop) != cudaSuccess || stop == -1) {
    failed = "cudaEventCreate";
    goto error_exit;
  }
