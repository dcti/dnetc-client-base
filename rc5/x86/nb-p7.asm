...
%macro ROUND_1_EVEN 1
        ...
        add     edx, edi
        add     ebp, edx
        rol     ebp, dl                     ; Llo4 = ROTL (Llo4 + A4 + Lhi4, A4 + Lhi2);
        mov     [work_Lhi4], edi
        mov     edi, [work_Llo3]
        mov     S3(%1), ecx
        add     ecx, esi
...
%macro ROUND_1_ODD 1
        ...
        add     eax, esi
        add     ecx, eax
        rol     ecx, al                     ; Lhi1 = ROTL (Lhi1 + A1 + Llo1, A1 + Llo1);
        mov     eax, S3(%1)
        mov     [work_Llo1], esi
        mov     esi, [work_Lhi3]
        mov     S2(%1), ebx
        add     eax, edi
        add     esi, eax
        rol     esi, al                     ; Lhi3 = ROTL (Lhi3 + A3 + Llo3, A3 + Llo3);
        mov     ebp, [work_Llo2]
        mov     S4(%1), edx
        mov     edx, [work_Lhi4]
        lea     eax, [ebx+ebp]
        add     edx, eax
        rol     edx, al                     ; Lhi2 = ROTL (Lhi2 + A2 + Llo2, A2 + Llo2);
        mov     ebp, [work_Llo4]
        mov     eax, S4(%1)
        mov     edi, [work_Lhi4]
        add     eax, ebp
        add     edi, eax
        rol     edi, al                     ; Lhi4 = ROTL (Lhi4 + A4 + Llo4, A4 + Llo4);
        mov     eax, S1(%1)
        lea     eax, [S_not(%1+1)+eax+ecx]
%endmacro
...
;        use1  use2        use1  use2
;        -----|-----|      -----|-----|
;   eax  A1    Llo4   ebx  A2    tmp
;   ecx  A3    Lhi1   edx  A4    Lhi2  
;   esi  Llo1  Lhi3   edi  Llo2  Lhi4
;   ebp  Llo3        
        ...
        mov     [work_A3], eax
        mov     [work_A4], eax
        lea     eax, [S_not(2)+eax+ecx]     ;  A1 = (A1 + Lhi1 + S_not(N));
...
_end_round1_p7:
        ...
        add     ebp, ebx
        rol     ebp, bl                     ; Llo4 = ROTL (Llo4 + A4 + Lhi4, A4 + Lhi4);
        mov     [work_Lhi4],edi
        mov     edi, [work_Llo3]
        lea     ebx, [ecx+esi]
        add     ecx, [work_pre2_r1]
        add     edi, ebx
        rol     edi, bl                     ; Llo3 = ROTL (Llo3 + A3 + Lhi3, A3 + Lhi3);              
        mov     ebx, S2(0)
        ...
        mov     ebp, [work_Llo2]
        add     ebp, ebx
        rol     ebp, bl                     ; Llo2 = ROTL (Llo2 + A2 + Lhi2, A2 + Lhi2);
        mov     [work_A3],ecx
        mov     ecx, [work_Lhi1]
        lea     ebx, [eax+ecx]
        mov     [work_Lhi3], esi
        ...
        add     esi, ebx
        rol     esi, bl                     ; Llo1 = ROTL (Llo1 + A1 + Lhi1, A1 + Lhi1);
        mov     ebx, [work_A2]
