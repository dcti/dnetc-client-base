...
;--------------------
	...
	XDEF	_rc5_unit_func_040

	INCLUDE	rc5-0x0-jg.i

;--------------------
;@(#)$Id: rc5-040-jg.s,v 1.1.2.1 2000/06/05 15:20:46 oliver Exp $

	; $VER: MC68040 RC5 core 24-Jan-2000
	;
	; MC680x0 RC5 key checking function
	; Dual key, unrolled loops, 040 optimised
	;
...
_rc5_unit_func_040:
	...
	lea	ruf040_vars,a6	;[ ] a6=core variables
	...
	lea	ruf040_vars+RUFV_L0X,a1	;[ ] a1=vars for loop
	move.l	d2,RUFV_AX(a6)	;[ ] Set Ax
	move.l	d2,d0	;[ ] d0=Ax
	lea	ruf040_sarray+8,a2	;[ ] a2=&Sa[02]
	add.l	d1,d2	;[ ] d2=L0x+Ax
	lea	ruf040_sarray+104+8,a3	;[ ] a3=&Sb[02]
	move.l	d2,RUFV_L0XAX(a6)	;[ ] Set L0x+Ax
	add.l	#P2Q,d0	;[ ] d0=Ax+P+2Q
	move.l	d0,RUFV_AXP2Q(a6)	;[ ] Set Ax+P+2Q

	RUF_ALIGN	_rc5_unit_func_040,d1	;Align to 8 bytes
ruf040_mainloop:
	;A : d0 d4
	;L0: d1 d5
	;L1: d2 d6
	...
	lea	ruf040_sarray+104,a3	;[ ] a3=Sb[] array
	...
	add.l	d3,d1	;[ ] d1=L0a=L0a+Aa+L1a
	lea	ruf040_sarray+8,a2	;[ ] a2=next iter &Sa[02]
	...
	bne.s	ruf040_notfounda	;[ ] Skip if not
...
ruf040_notfounda:
	...
	bne.s	ruf040_notfoundb	;[ ] Skip if not
...
ruf040_notfoundb:
	...
	bcc.s	ruf040_midone
	addq.b	#1,20(a0)	;Every 256^4 = 2^32 = (4294967296)
	bcc.s	ruf040_l0chg
	addq.b	#1,21(a0)	;Every 256^5 = 2^40 = (1099511627776)
	bcc.s	ruf040_l0chg
	addq.b	#1,22(a0)	;Every 256^6 = 2^48 =  (281474976710656)
	bcc.s	ruf040_l0chg
	addq.b	#1,23(a0)	;Every 256^7 = 2^56 =  (72057594037927936)

	; Need to do anything special wrapping 0xff..f -> 0x00..0 ?

ruf040_l0chg:	; L0 has changed so recalculate constants

	add.l	a5,d1	;d1=L0=L0+S[00]
	ror.l	#3,d1	;d1=L0=(L0+A)>>>3 = (L0+A)<<<P0QR3
	move.l	d1,(a1)	;Set L0x

	add.l	#PR3Q,d1	;d1=A+P+Q
	rol.l	#3,d1	;d1=(A+P+Q)<<<3
	move.l	d1,RUFV_AX(a6)	;Set Ax

	add.l	(a1),d1	;d1=Ax+L0x
	move.l	d1,RUFV_L0XAX(a6)	;Set Ax+L0x

	move.l	RUFV_AX(a6),d1	;d1=Ax
	add.l	#P2Q,d1	;d1=A+P+2Q
	move.l	d1,RUFV_AXP2Q(a6)	;Set A+P+2Q

	RUF_ALIGN	_rc5_unit_func_040,d2	;Align to 8 bytes
ruf040_midone:	subq.l	#1,(a7)	;Loop back for next key
	bne	ruf040_mainloop
...
ruf040_check64:
	...
	lea	ruf040_sarray(pc),a1	;a1=aligned S[] storage

	;---- Mix secret key into S[] ----

	move.l	20(a0),d1	;d1=L0=L0.lo
	lea	ruf040_p2q(pc),a3	;a3=&S[02] read address
...
;--------------------
	...
	CNOP	0,8
ruf040_p2q:	;Table of P+nQ values for 2<=n<=26
