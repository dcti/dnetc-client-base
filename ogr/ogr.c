...
/*-----------------------------------------*/
static void found_one(struct State *State)
{
  printf("found one\n");
  ...
  (*dispatch_table.result)(State, State->marks, State->maxdepth);
}
...
static int ogr_result(void *state, void *result, int resultlen)
{
  printf("Need to override result()!\n");
  return CORE_E_STOPPED;
}
...
CoreDispatchTable *ogr_get_dispatch_table()
{
  ...
  dispatch_table.result  = &ogr_result;
