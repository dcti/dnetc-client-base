...
#define __ASM_X86_H__ "@(#)$Id: asm-x86.h,v 1.3 2008/02/26 20:45:18 kakace Exp $"
...
#define STATIC_ASSERT(cond)  { typedef int safoo[(cond) ? 1 : -1]; }
  ...
  #define __CNTLZ(x) ((x) == 0xFFFFFFFF ? 33 : __CNTLZ__(x))
      ...
      && !defined(OGROPT_64BIT_IMPLEMENTATION)
  ...
  #if 0 && (OGROPT_ALTERNATE_CYCLE == 0) && (OGROPT_ALTERNATE_COMP_LEFT_LIST_RIGHT == 1) \
        && !defined(OGROPT_64BIT_IMPLEMENTATION)
    #define COMP_LEFT_LIST_RIGHT(lev,s) { \
      __asm__ volatile(                   \
        "movl  4(%2),%%eax\n\t"           \
