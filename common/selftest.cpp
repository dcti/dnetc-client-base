...
return "@(#)$Id: selftest.cpp,v 1.68 2002/09/23 12:01:32 acidblood Exp $"; }
...
// RC5-32/12/9 test cases -- generated by gentests64.cpp:
...
long SelfTest( unsigned int contest )
{
        ...
        threadpos++ )
  {
    ...
    for ( testnum = 0 ; !userbreak && testnum < TEST_CASE_COUNT ; testnum++ )
    {
      ...
      u32 expectedsolution_hi, expectedsolution_lo, expectedsolution_vlo;
      ...
      if (contest == RC5_72)
      {
        ...
        expectedsolution_vlo = (*test_cases)[testnum][0];
        expectedsolution_lo  = (*test_cases)[testnum][1];
        expectedsolution_hi  = (*test_cases)[testnum][2];
        contestwork.bigcrypto.key.vlo = expectedsolution_vlo;
...
//                PROJECT_NOT_HANDLED(contest);
                if ( resultcode != RESULT_FOUND )                /* no solution */
                {
                  contestwork.bigcrypto.key.vlo =
                  contestwork.bigcrypto.key.lo = contestwork.bigcrypto.key.hi = 0;
                  resulttext = "FAILED";
                  resultcode = -1;
                }
                else if (contestwork.bigcrypto.key.vlo != expectedsolution_vlo ||
                ...
                LogScreen( "\r%s: Test %02d %s: %02X:%08X:%08X-%02X:%08X:%08X\n",
                   contname, testnum + 1, resulttext, contestwork.bigcrypto.key.vlo, 
                   contestwork.bigcrypto.key.hi, contestwork.bigcrypto.key.lo,
                   expectedsolution_vlo, expectedsolution_hi, expectedsolution_lo );
                break;
              }
              #ifdef HAVE_OGR_CORES
              case OGR:
