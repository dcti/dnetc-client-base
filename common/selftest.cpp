...
return "@(#)$Id: selftest.cpp,v 1.52 1999/11/08 02:02:44 cyp Exp $"; }
...
static const u32 csc_test_cases[TEST_CASE_COUNT][8] = {
...
int SelfTest( unsigned int contest )
{
        ...
        threadpos++ )
  {
    ...
    for ( testnum = 0 ; !userbreak && testnum < TEST_CASE_COUNT ; testnum++ )
    {
      ...
      if (contest == OGR)
      {
        test_cases = (const u32 (*)[TEST_CASE_COUNT][8])&ogr_test_cases[0][0];
        expectedsolution.lo = (*test_cases)[testnum][0];
      }
#endif
#ifdef HAVE_CSC_CORES
