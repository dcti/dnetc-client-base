...
/* ----------------------------------------------------------------------- */
/* support functions for Problem Info Accessors                            */
/* ----------------------------------------------------------------------- */
...
{
    // XXX if it's public data, why the heck do we have an accessor function?
    InternalProblem *thisprob = __pick_probptr(__thisprob, PICKPROB_MAIN);
    return thisprob->pub_data.contest;
}
...
unsigned int ProblemGetCPermille(void *__thisprob, int permille_only_if_exact)
{
    ...
    if( !permille_only_if_exact )
    {
      if (!thisprob->priv_data.started)
        retval = thisprob->pub_data.startpermille;
      else if (rescode != RESULT_WORKING) /* _FOUND or _NOTHING */
        retval = 1000;
      else
        retval = __compute_permille(contestid, &work); 
    }
    return retval;
}
    ...
    if (rescode >= 0) /* hmm! */
    {
        ...
        switch (contestid)
        {
          ...
          { 
            ...
            if (tcounthi || tcountlo) /* only if finished */
            {
              /* note that we return zero for test packets */
        ...
        switch (contestid)
        {
          ...
          { 
            ...
            sprintf( scratch, "%08lX:%08lX:%u*2^%u", 
                     ...
                     units, twoxx );
            strncpy( buf, scratch, bufsz );
            buf[bufsz-1] = '\0';
          } /* case: crypto */
          break;
#ifdef HAVE_OGR_CORES
          case OGR:
              ogr_stubstr_r( &work.ogr.workstub.stub, buf, bufsz, (cwp ? work.ogr.workstub.worklength : 0));
          break;
#endif /* HAVE_OGR_CORES */
          default:
          break;  
...
unsigned int ProblemGetRate(void *__thisprob, char *ratebuf, unsigned int ratebufsz)
{
    ...
    ProblemGetElapsedTime(__thisprob, &e_sec, &e_usec);
    ...
    if (rescode >= 0) /* hmm! */
    {
        ...
        ProblemComputeRate( contestid, e_sec, e_usec, ccounthi, ccountlo, &lo, ratebuf, ratebufsz );
    ...
    if (rescode >= 0) /* hmm! */
    {
        ...
        switch (contestid)
        {
          ...
          case OGR:
          {
            if (rescode == RESULT_NOTHING || rescode == RESULT_FOUND)
            {
              tcounthi = work.ogr.nodes.hi;
              tcountlo = work.ogr.nodes.lo;
            }
          } /* OGR */      
          break;
#endif /* HAVE_OGR_CORES */
...
void ProblemGetTBuf(void *__thisprob, int numstring_style, char *tcountbuf, unsigned int tcountbufsz)
{
    ...
    {
        u32 tcounthi=0, tcountlo=0; /*total 'iter' (n/a if not finished)*/
        const char *numstr_suffix = (contestid == RC5 || 
                                     contestid == DES ||
                                     contestid == CSC) ? "keys" : "nodes";

        ProblemGetTCounts(__thisprob, &tcounthi, &tcountlo);
...
void ProblemGetCBuf(void *__thisprob, int numstring_style, char *ccountbuf, unsigned int ccountbufsz)
{
    ...
    if (rescode >= 0) /* hmm! */
    {
        u32 ccounthi=0, ccountlo=0; /*total 'iter' (n/a if not finished)*/
        const char *numstr_suffix = (contestid == RC5 || 
                                     contestid == DES ||
                                     contestid == CSC) ? "keys" : "nodes";

        ProblemGetCCounts(__thisprob, &ccounthi, &ccountlo);
...
void ProblemGetDBuf(void *__thisprob, int numstring_style, char *dcountbuf, unsigned int dcountbufsz)
{
    ...
    if (rescode >= 0) /* hmm! */
    {
        u32 dcounthi=0, dcountlo=0; /*total 'iter' (n/a if not finished)*/
        const char *numstr_suffix = (contestid == RC5 || 
                                     contestid == DES ||
                                     contestid == CSC) ? "keys" : "nodes";

        ProblemGetDCounts(__thisprob, &dcounthi, &dcountlo);
