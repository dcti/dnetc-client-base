...
//
// $Log: csc-1key-bitslicer-mmx.cpp,v $
// Revision 1.1  1999/12/08 05:35:45  remi
// Files moved from /csc to /csc/x86/mmx
//
// Revision 1.1.2.1  1999/11/23 23:37:38  remi
// Initial commit.
//
//
...
return "@(#)$Id: csc-1key-bitslicer-mmx.cpp,v 1.1 1999/12/08 05:35:45 remi Exp $"; }
#endif
...
#define APPLY_Ms(adr, adl)						\
  ...
  printf( fmt,  xy70, x0^y1, xy12, x2^y3, xy34, x4^y5, xy56, x6^y7 );	\
  csc_transP_call(							\
...
# ----------------------------------------------------
  ...
  " : "=b"(skp)								\
    : "D"(x_y_xy), "a"(cfr), "d"(csc_params), "m"(mmNOT), "0"(skp)	\
    : "%ecx", "%esi"							\
  );									\
  } while( 0 )
