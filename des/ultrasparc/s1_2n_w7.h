...
/*
 * $Log: s1_2n_w7.h,v $
 * Revision 1.2  1998/06/14 15:18:53  remi
...
/* upon entry here, needs a3, a5, a4, a6 valid */
	...
	ASM_F_XOR (gx49, gx48, gx46);		/* A              */  /* F */

	ASM_AND_NOT (x1, a3, a5);		/*               */
	ASM_AND_NOT (x3, a3, a4);		/*               */
	ASM_D_LOAD (a2, in[S12]);
	ASM_F_OR (gx39, gx38, gx30);		/*     E          */  /* F */

	ASM_OR (x4, x3, a5);			/*               */
	ASM_XOR (x8, a3, a4);			/*               O */
	ASM_D_LOAD (k2, ((INNER_LOOP_SLICE *)key2)[0]);	
	ASM_F_OR_NOT (gx40, g_a1, gx39);	/*     E          */  /* F */

	ASM_XOR (x2, x1, a4);			/*               */
	ASM_AND (x5, a6, x4);			/*               */
	ASM_A_LOAD (key1, offsets->Key_Ptrs[OFFSET1 + 1]);
	ASM_F_XOR (gx41, gx37, gx40);		/*     EF         */  /* F */

	ASM_XOR (x6, x2, x5);			/*               */
	ASM_XOR (x13, a5, x5);			/*               O */
	ASM_D_LOAD (a1, in[S11]);
	ASM_F_XOR (Preloadg_3, Preloadg_3, gx41); /*     E          */  /* F */

	ASM_AND (x14, x13, x8);			/*               O */
	ASM_AND_NOT (x15, a5, a4);		/*              N  */
	ASM_D_LOAD (k1, ((INNER_LOOP_SLICE *)key1)[0]);
	ASM_F_XOR (gx47, gx43, gx46);		/*  B D           */  /* F */

/**/	ASM_AND_NOT (x7, a4, a5);		/*               */
	ASM_XOR (x16, x3, x14);			/*              N  */
    ASM_F_D_STORE (out[D71], Preloadg_1);	/*            L   */  /* F */
	ASM_F_OR (gx50, g_a1, gx49);		/* A C            */  /* F */

	ASM_XOR (a2, a2, k2);
	ASM_OR (x28, x6, x7);			/*             M  */
    ASM_F_D_STORE (out[D72], Preloadg_2);	/*         I      */  /* F */
	ASM_F_XOR (gx51, gx47, gx50);		/* AB             */  /* F */

	ASM_XOR (a1, a1, k1);
	ASM_OR (x17, a6, x16);			/*              N  */
    ASM_F_D_STORE (out[D73], Preloadg_3);	/*     E          */  /* F */
	ASM_F_XOR (Preloadg_4, Preloadg_4, gx51); /* A              */  /* F */

	ASM_AND_NOT (x9, a6, x8);		/*               O */
	ASM_XOR (x18, x15, x17);		/*              N  */
    ASM_F_D_STORE (out[D74], Preloadg_4);	/* A              */  /* F */
...
/**/	ASM_AND (x34, a1, x33);			/*           K    */
	...
	ASM_OR (x55, a1, x54);			/* A              */
    ASM_D_LOAD (t_a6, in[S26]);				/* PIPELINE, shadow D11 */
    ...
    ASM_COMMENT_END_INCLUDE (end_of_s1_2n_w7);
